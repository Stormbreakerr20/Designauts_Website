...
const Artworks = ({textLeave,textEnter}) => {
  ...
  const titleSpring = (show) =>  useSpring({
    ...
    letterSpacing: show ? "0.1em" : windowWidth < 800 ? "0.5em" : "0.8em",
  });

  const descSpring = (show) => useSpring({
    opacity: show ? 1 : 0,
    transform: show ? "scale(1) " : "scale(1.5)",
    letterSpacing: show ? "0.1em" : windowWidth < 800 ? "0.3em" : "0.8em",
  });
  ...
  return (
    <div className=" text-white items-center ">
      <div className=" overflow-hidden">
        <div onMouseEnter={textEnter} onMouseLeave={textLeave} className="flex max-md:text-5xl max-sm:text-4xl justify-center items-center text-6xl h-[100vh] mb-[15rem]">
          Welcome to Artworks
        </div>
        <div className="flex flex-col gap-[10rem] mx-auto relative max-xl:max-w-[80%] h-[400vh] max-md:gap-5">
        {ArtWorksArr.map((artwork, index) => {
          const show = (scrolled > (380 + index*500 + (windowWidth > 1500 ? - index*20 : windowWidth > 800 ? index*10 : (index + 1)*100 - 200*index ))) && (scrolled < ((380 + index*500) + (windowWidth > 1500 ? 300 : windowWidth > 800 ?  350 + 20*index : 400 - 100*index)));
          const margin_val = ((windowWidth > 1900 ? 200 : (windowWidth < 1050 ? (windowWidth < 750 ? 200 : 300) : 300)  )*index).toString() + "px";
          return (
            <>
            <div key={index} className={`fixed z-30 top-[50vh] -rotate-90 left-0  overflow-visible`} >
            <animated.div style={dateSpring(show)} >
              <span onMouseEnter={textEnter} onMouseLeave={textLeave} className="text-8xl max-xl:text-6xl max-sm:text-4xl">{artwork.date}</span>
            </animated.div>
            </div>
            <div
            key={index}
            ...
            <div key={index} className='w-[100%] flex justify-end -rotate-12' style={{ marginLeft: margin_val }}>
              <animated.div style={imageSpring(300 + index*500 + 400,0 )}>
                <img
                  src={artwork.img}
                  alt="" 
                  className="h-[500px] w-[500px] max-lg:scale-[90%] max-xl:ml-[40rem] max-md:ml-[30rem] max-md:scale-75 rounded-lg brightness-75 z-0"
                />
              </animated.div>
            </div>
          </>
